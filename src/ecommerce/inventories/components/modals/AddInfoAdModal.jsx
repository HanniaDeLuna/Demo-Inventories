...
const AddInfoAdModal = ({AddInfoAdShowModal, setAddInfoAdShowModal}) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
          ...
          await AddOneInfoAd(values.IdAlmacenOK,InfoAd);
  ...
  const fetchAlmacenes = async () => {
      try { 
          ...
          const InfoAdData = data.flatMap(inventario => inventario.negocios.flatMap(negocio => negocio.almacenes));
          setAlmacenes(InfoAdData); 
      } catch (error) { 
          console.error("Error al obtener informaciones adicionales:", error); 
      }
  };
  ...
  return (
    ...
    >
      <form onSubmit={formik.handleSubmit}>
        ...
        >
        <Select
          id="IdAlmacenOK" name="IdAlmacenOK"
          value={formik.values.IdAlmacenOK}
          onChange={formik.handleChange}
          onBlur={formik.handleBlur}
          error={formik.touched.IdAlmacenOK && Boolean(formik.errors.IdAlmacenOK)}>
          <MenuItem value="">
              <em>Seleccione una almacén</em>
          </MenuItem>
          {Almacenes.map((almacen) => (
              <MenuItem key={almacen.IdAlmacenOK} 
              value={almacen.IdAlmacenOK}>
              {almacen.IdAlmacenOK}
              </MenuItem>
          ))}
        </Select> {formik.touched.IdAlmacenOK && formik.errors.IdAlmacenOK}
